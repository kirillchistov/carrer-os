...
| `DATABASE_URL` | Vercel + local | **Transaction pooler**, `*.pooler.supabase.com:6543`, user `postgres.<project-ref>`. Direct `db.*` is rewritten at runtime if `SUPABASE_REGION` is set |
| `SUPABASE_REGION` | Vercel + local | AWS region of the Supabase project (e.g. `eu-west-3`). Needed to rewrite an old `db.*` URL |
...
5. **Database URL.** Prefer the Supabase **Transaction pooler** URI (`aws-0-<region>.pooler.supabase.com`, port `6543`, user `postgres.<project-ref>`). If Vercel still has the old `db.<project>.supabase.co` URL, set `SUPABASE_REGION` (this project: `eu-west-3`) — the app rewrites it. Do not put `sslmode=require` on the URL.
