...
| `DATABASE_URL` | Vercel + local | Pooled Postgres (port 6543) |
...
5. **Confirm email.** If confirmation is on, signup does **not** enter the app until the email link is opened. The UI says to check mail.
6. **Logs.** Server logs emit `{"type":"auth_event",…}` with `event`, `ok`, `reason` — no email, no tokens. Sentry captures unhandled errors when `SENTRY_DSN` is set.
